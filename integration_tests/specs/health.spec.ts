...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), manageAResidentApi.stubPing(), tokenVerification.stubPing()])
    })
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-approved-premises-manage-a-resident-ui')
    })
  })

  test.describe('Some unhealthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), manageAResidentApi.stubPing(), tokenVerification.stubPing(500)])
    })
