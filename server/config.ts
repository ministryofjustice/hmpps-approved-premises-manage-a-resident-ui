...
export default {
  ...
  apis: {
    ...
    manageAResidentApi: {
      url: get('MANAGE_A_RESIDENT_API_URL', 'http://localhost:8080', requiredInProduction),
      healthPath: '/health/ping',
      timeout: {
        response: Number(get('MANAGE_A_RESIDENT_API_TIMEOUT_RESPONSE', 5000)),
        deadline: Number(get('MANAGE_A_RESIDENT_API_TIMEOUT_DEADLINE', 5000)),
      },
      agent: new AgentConfig(Number(get('MANAGE_A_RESIDENT_API_TIMEOUT_RESPONSE', 5000))),
    },
  },
  sqs: {
    audit: auditConfig(),
